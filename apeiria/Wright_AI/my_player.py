...
import sys
import os  
from cshogi import move_from
# Shogi まで戻ってから、python-dlshogi2 を通らず pydlshogi2 が見えるようにする
python_module_root = os.path.abspath(os.path.dirname(__file__))

sys.path.append(python_module_root)

from cshogi import Board, BLACK, NOT_REPETITION, REPETITION_DRAW, REPETITION_WIN, REPETITION_SUPERIOR, move_to_usi
from module.features import FEATURES_NUM, make_input_features, make_move_label
from module.uct.uct_node import NodeTree
from module.network.policy_value_resnet import PolicyValueNetwork
from module.player.base_player import BasePlayer
...
with open(log_path, "a", encoding="utf-8") as f:
    ...
    f.write(f"[{now}] [log reset request] 次の対局の準備のためログを初期化します\n")

# 少し待つ（監視プロセスがこの記録を読み取るための猶予）
time.sleep(0.5)
...
class MCTSPlayer(BasePlayer):
    # USIエンジンの名前
    name = 'python-mymodule'
    # デフォルトチェックポイント
    DEFAULT_MODELFILE = 'brain/checkpoint-005.pth'
    ...
    def position(self, sfen, usi_moves):
        ...
        # log(f"[position] sfen={sfen}, moves={usi_moves}")
        if self.debug:
    ...
    def go(self):
        ...
        if "*" in bestmove_str:
            # 打ち駒の場合
            piece_code, to_sq = bestmove_str.split("*")
            USI_PIECE_TO_NAME = {
                "P": "歩", "L": "香", "N": "桂", "S": "銀",
                "G": "金", "B": "角", "R": "飛"
            }
