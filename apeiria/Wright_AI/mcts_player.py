...
class MCTSPlayer(BasePlayer):
    ...
    # デフォルトチェックポイント
    DEFAULT_MODELFILE = r"C:\Python\apeiria\apeiria\Wright_AI\checkpoint-005.pth"  # ✅ これならOK
    ...
    def _set_position(self, sfen, usi_moves):
        ...
        print("[DEBUG] 現在の盤面（AIの認識）:")
        self.print_human_readable_board(self.root_board)
        write_log("ai_board.log", "AIの認識", self.root_board)

        # # デバッグ時の最終確認
        # if self.debug:
        #     square_8f = (9 - 8) * 9 + (ord('f') - ord('a'))
        #     raw = self.root_board.pieces[square_8f]
        #     if raw != 0:
        #         piece_type = raw & 0b00011111
        #         color = '先手' if raw & 0b00100000 else '後手'
        #         print(f"[DEBUG] 最終8fの駒: raw={raw} / 所有: {color} / 駒種: {piece_type}", flush=True)
        #     else:
        #         print("[DEBUG] 最終8fの駒: なし", flush=True)
    ...
    def run(self):
        ...
        while True:
            ...
            if line.startswith("usi"):
                ...
                if piece:
                    print(f"[DEBUG] 8fの駒: {piece} 先手か: {piece.color == BLACK} 駒種: {piece.piece_type}", flush=True)
                else:
                    print("[DEBUG] 8fの駒: なし", flush=True)
